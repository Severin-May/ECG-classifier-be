"""
@brief: this file contains all different models
"""
import torch
from torch import nn
from ECG_pytorch.layers import ada_hermite, vp_layer_apr, vp_layer_coeffs;

"""
@brief: NeuralNetwork class implements the model
"""
...
class VPNet4(nn.Module):
    def __init__(self, penalty=15.0):
        super().__init__()
        self.linear1 = nn.Linear(4, 150) # free params are the matrices by which we multiply the input
        self.relu = nn.ReLU() # non-linear mapping
...
class VPNet12(nn.Module):
    def __init__(self, penalty=5.0):
        super().__init__()
        self.linear1 = nn.Linear(12, 150) # free params are the matrices by which we multiply the input
        self.relu = nn.ReLU() # non-linear mapping
        self.linear2 = nn.Linear(150, 150)
        self.linear3 = nn.Linear(150, 1)
        self.sigmoid = nn.Sigmoid()
...
class FCNNNet(nn.Module):
    def __init__(self, penalty=5.0, device=None):
        super().__init__()
